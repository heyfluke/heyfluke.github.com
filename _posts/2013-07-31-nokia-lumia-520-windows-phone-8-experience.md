...
[更新：经过仔细使用，发现其实和activity应该差不多，程序也是由一个一个类似概念的画面组成（由于没有看过SDK文档，不知道具体的概念是什么），通过后退导航得到的结果也是类似网页浏览历史记录一样的（在Android下面历史记录有几种摆放方式，还可以认为打乱历史，因此使用起来有时候无法预期，不确定WP8是否也是如此）。并且比起Android，更加自然的一点是桌面也相当于一个普通的历史节点，可以继续后退。长按后退会出现整个历史列表。]
