...
[更新：经过仔细使用，发现其实和activity应该差不多，程序也是由一个一个类似概念的画面组成（由于没有看过SDK文档，不知道具体的概念是什么），通过后退导航得到的结果也是类似网页浏览历史记录一样的（历史记录可以通过跳转的方式来打乱，从列表回到某个历史相当于把历史节点抽出来查到最后，不过如果不人为打开历史节点并且打乱的话，一般还是可以预期的，而Android下面似乎比较混乱，还有可能造成循环）。并且比起Android，更加自然的一点是桌面也相当于一个普通的历史节点，可以继续后退。长按后退会出现整个历史列表。]
...
[再更新：长按后退键会出现类似别的系统里面的任务管理界面--但实际上是历史列表，这和后退键的功能匹配得非常和谐啊。]
...
[更新：经过几天的正常使用，挖掘出了一些新的功能，但更加重要的是，越来越觉得这个系统是一个十分和谐、有设计偏见、体验流畅的系统，虽然软件上面还不能和iOS以及Android阵营匹配，但相信这只是2010年的Android，期待明年后年的发力，越来越精彩。]
